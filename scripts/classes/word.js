...
export default class Word {
  ...
  highlightWord(cursor_position, container) {
    for (
      let i = cursor_position + 1;
      i < cursor_position + this.text.length + 1;
      i++
    ) {
  ...
  validateWord(end, container, input) {
    if (this.text === input) {
      ...
      while (i < end) {
        if (this.text[i]) {
          container.childNodes[i + this.firstCharPosition].instance.text =
            this.text[i]
          container.childNodes[
            i + this.firstCharPosition
          ].instance.crossCharacter()
          i += 1
        } else {
  ...
  invalidateWord(container, input) {
    if (input.length < this.text.length) {
      for (
        let i = this.firstCharPosition;
        i < this.firstCharPosition + this.text.length;
        i++
      ) {
  ...
  replaceOriginalWord(container, input) {
    let i = 0
    let end = input.length
    while (i < end) {
      if (this.text[i]) {
