...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    const hiddenParagraph = document.getElementById('paragraph__hidden-instructions')
    ...
    seeProgressButton.addEventListener('click', () => open('/chart.html', '_self'))
    ...
    function validateCharInput(event) {
        ...
        if (key) {
            if (key === 'Escape') {
                ...
                return
            } else if (key === 'Backspace' && cursor.element.previousSibling.innerText === ' ') {
                return
            } else if (key === 'Backspace' && counters.overflow > 0) {
                handleBackspaceWithOverflow(cursor, letterNodes, container, counters)
            } else if (key === 'Backspace' && counters.overflow === 0) {
                ...
                handleExpectedSpaceCharacter(cursor, counters, word, letterNodes, container)
                jumpToNextWord()
            } else if (key === 'Space') {
                handleUnexpectedSpaceCharacter(cursor, counters, word, letterNodes, container)
                jumpToNextWord()
            } else if (text[counters.charPosition] === ' ' && key !== 'Space') {
    ...
    function jumpToNextWord() {
        word.changeExpectedWord(word.position + 1, words[word.position + 1], cursor.position)
        counters.input = ''
